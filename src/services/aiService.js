function analyzeRoute(routeData, weather, traffic) {
    let score = 10;
    let risk_level = "low";
    ...
    if (weatherDescription.includes("storm") || weatherDescription.includes("thunder")) {
        score -= 4;
    }
    ...
    if (weatherDescription.includes("fog") || weatherDescription.includes("mist")) {
        score -= 2;
    }

    // Impact trafic
    if (traffic?.congestion_level === "medium") {
        score -= 2;
    }
    ...
    if (traffic?.jam_factor !== null && traffic?.jam_factor !== undefined) {
        if (traffic.jam_factor >= 8) {
            score -= 2;
        } else if (traffic.jam_factor >= 5) {
            score -= 1;
        }
    }
